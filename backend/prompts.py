...
from .test_context import format_test_context_block
...
def build_conversation_context(
	...
	for message in conversation:
		...
		if role == "ASSISTANT":
			...
			test_ctx = message.get("test_context")
			if isinstance(test_ctx, list) and test_ctx:
				block = format_test_context_block(test_ctx)
				if block:
					content = f"{content}\n\n{block}" if content else block
		lines.append(f"{role}: {content}")
	lines.append("\n## Current Gorgias program in the editor")
	if current_code.strip():
		lines.append(current_code)
	else:
		lines.append("(empty — no program yet)")
	lines.append(
		"\nRespond to the user's latest message.\n"
		"- If they attach test results, use those details (query, facts, outcome, explanations, program used) "
		"together with the current editor program when answering.\n"
		"- If they ask a question (about Gorgias in general, how something works, or about this program): "
		"answer clearly in \"message\" using Markdown. Keep \"code\" identical to the current program above unless they also ask for edits.\n"
		"- If they ask to create, translate, or change a scenario: produce the updated full program in \"code\" "
		"following the spec only — generate from scratch when the program is empty; do not copy boilerplate "
		"or example programs unless the user provided them.\n"
		"- If the program is empty and they only ask a conceptual question, leave \"code\" as an empty string.\n"
		"Return only valid JSON with keys \"message\" and \"code\"."
	)
	if syntax_errors:
		lines.append("\n## Syntax validation failed — fix every error")
		for err in syntax_errors:
			lines.append(f"- {err.get('message', err)}")
		lines.append(
			"Return corrected full program in \"code\" (same JSON keys). "
			"Preserve the user's intent from the conversation."
		)
	return "\n".join(lines)
...
def generate_system_prompt() -> str:
	return (
		"You are an expert assistant for the Gorgias argumentation framework. "
		"You help users in two ways:\n"
		"1) Answer questions — explain Gorgias concepts, syntax, rule levels (r/p/c), preferences, complements, "
		"or walk through the user's current program when one is provided.\n"
		"2) Author programs — create or edit Gorgias Prolog from natural language using ONLY this spec:\n"
		f"{GORGIAS_SPEC_2}\n"
		"IMPORTANT MINIMALITY RULE (when editing code): The final code must satisfy the user's requests. "
		"Remove unrelated rules, facts, complements, and abducibles that are not required. "
		"When creating a new program, generate only from the spec — never insert a canned example or starter template.\n"
		"Response MUST ONLY be valid JSON. No text outside JSON:\n"
		'- "message": User-facing text in Markdown (headings, lists, **bold**, `inline code`, fenced ```prolog blocks '
		"for program excerpts). For questions, give a clear helpful answer. "
		"For code-only updates without a question, a brief technical summary (1 sentence). "
		"Do not wrap the whole JSON in markdown fences.\n"
		'- "code": The full current program after your turn. When the user only asked a question and did not '
		"request changes, return the same program verbatim (or empty string if there is no program yet). "
		"When they requested changes, return complete syntactically valid Gorgias Prolog."
	)